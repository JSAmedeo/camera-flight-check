...
**Camera Flight Check** (folder: `Camera Pre Flight Utility`) is a standalone Windows desktop utility that seasonal photo-set operators run before each shift at Cherry Hill Programs venues (Santa/Bunny sets). It walks the operator through a ~4-minute readiness check: operator sign-in → station walk-around → camera detection → grey-card calibration (auto-corrects the camera's white balance + exposure in-camera) → guided test-photo QA → hand off to RPS (the photo sales app).
...
- **Strings:** ALL operator-facing copy lives in `strings.js` (`window.CFC_STRINGS`, loaded before the JSX) — training materials change, so **never hardcode UI text in JSX**; add a key to strings.js and reference `S.section.key` (templates use `{placeholders}` via the `fmt()` helper). Walk-around tutorial images auto-load from `assets/tutorials/<key>-<n>.png` (see the README there) with a named placeholder until each file exists.
...
- **Run log:** check-run events append to `%APPDATA%\Camera Flight Check\check-runs.jsonl` (`runs:save` IPC, `window.cfc.runs.save`). Records are API-shaped (`check_started`/`check_completed` with runId, operator, ISO timestamps, duration) — a future phase POSTs them to the company API, so keep the schema stable and additive.
...
**Field validation on the in-house station PC, going well.** Canon T7 verified end-to-end: detect, settings (incl. mode/quality/SD capacity), capture/transfer, grey-card corrections on a real scene (ISO 6400 stress test passed), and the full guided QA loop exercised with real photos of a Santa print. Result tolerances field-tuned (1-stop exposure, visible-cast color, clipping guard). Camera-session reliability hardened after two field incidents (orphan kill + single-instance lock + persistent `camera-host.log`). A **real-person grey-card test is next** — collect: WB preset chosen, over/under-correction of the retake (tunes the warmth→Kelvin exponent `0.6` in `planCorrections`), whether the "strongly cool on WB Shade" anomaly recurs below the clipping guard, and how the 3/4-bust guide fits a real seated Santa. Canon T6 also validated (2026-07-17). Nikon D: completely untested but the DLL ships dedicated classes for 24+ D/Z models plus a NikonBase PTP fallback — expect one debugging session with the first real body (capture flow, settings timing, Auto-dial heuristic, WB vocabulary, and the Canon-only SD-capacity path all need Nikon equivalents/validation); supported-model list from the user still pending. Training-material pipeline is ready and waiting on content: copy edits go in `strings.js`, and the 18 walk-around tutorial photos drop into `assets/tutorials/` (named per its README) to replace the placeholders.
